...
export const emptyProfile = (): ActressProfile => ({
  disabled: [],
  enabledTiers: [true, true, true, true, true],
  revision: 1,
});

export function validateProfile(input: unknown): ActressProfile {
...
export function eligibleActresses(items: Actress[], profile: ActressProfile) {
  return items.filter(
    (item) =>
